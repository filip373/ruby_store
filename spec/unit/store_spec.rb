...
RSpec.describe Store do
  ...
  subject(:store) do
    Store.new(warehouse: Warehouse.new([product1, product1]),
              basket: Basket.new)
  end
  ...
  describe '#add_to_basket' do
    context 'when product is in warehouse' do
      it 'does not raise error' do
        expect { store.add_to_basket(product1) }.to_not raise_error
      end
      it 'adds product to basket' do
        store.add_to_basket(product1)
        expect(store.basket_state).to eql(product1 => 1)
      end
      it 'removes product from warehouse' do
        store.add_to_basket(product1)
        expect(store.warehouse_state).to eql(product1 => 1)
      end
    end

    context 'when product is not in warehouse' do
      it 'raises error' do
        expect { store.add_to_basket(product2) }.to raise_error(/Cannot remove/)
      end
    end
  end
  ...
  describe '#remove_from_basket' do
    context 'when product is in warehouse' do
      before { store.add_to_basket(product1) }
      it 'does not raise error' do
        expect { store.remove_from_basket(product1) }.not_to raise_error
      end
      it 'removes product from basket' do
        store.remove_from_basket(product1)
        expect(store.basket_state).to be_empty
      end
      it 'adds product to warehouse' do
        store.remove_from_basket(product1)
        expect(store.warehouse_state).to eql(product1 => 2)
      end
    end

    context 'when product is not in warehouse' do
      it 'raises error' do
        expect { store.remove_from_basket(product1) }.to raise_error(/Cannot remove/)
      end
    end
  end
  ...
  describe '#basket_state' do
    ...
    context 'when basket contains products' do
      ...
      it 'returns containing products' do
        ...
        expect(state).to include(product1 => 2)
      end
    end
  end
